 ...
 * Validates ads against Fynd brand guidelines
 */
...
export class ComplianceChecker {
  ...
  private checkBrandColors(objects: any[]): void {
    ...
    const brandColorValues = Object.values(FYND_COLORS).map(c => c.toLowerCase());
    const hasBrandColor = Array.from(colors).some(c => 
      brandColorValues.includes(c) || this.isCloseToFyndColor(c)
    );
    ...
    this.checks.push({
      ...
      message: hasBrandColor 
        ...
        : 'Consider using Fynd brand colors (Primary: #41E0D0, Secondary: #333333)',
      severity: hasBrandColor ? 'info' : 'warning',
      suggestion: !hasBrandColor ? 'Add Fynd Teal (#41E0D0) or Dark (#333333) to improve brand recognition' : undefined,
    });
  }
  ...
  private checkLogoPresence(objects: any[]): void {
    ...
    this.checks.push({
      ...
      suggestion: !hasLogo ? 'Add the Fynd logo for brand recognition' : undefined,
    });
  }
  ...
  /**
   * Check if a color is close to Fynd brand colors
   */
  private isCloseToFyndColor(hex: string): boolean {
    // Simple check - could be improved with actual color distance calculation
    const fyndPrimary = '#41E0D0';
    const fyndSecondary = '#333333';
    
    return hex.includes('41') && hex.includes('e0') || // Close to teal
           hex.includes('33');    // Close to dark
  }
}
